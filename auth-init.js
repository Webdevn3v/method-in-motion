...
function friendlyAuthError(error, action) {
  ...
  if (code === "auth/weak-password") return "Your password must contain at least six characters.";
  if (code === "auth/missing-password") return "Enter your password.";
  if (code === "auth/invalid-credential" || code === "auth/wrong-password" || code === "auth/user-not-found") {
    return "The email or password is incorrect.";
  }
  if (code === "auth/too-many-requests") return "Too many attempts. Wait a moment and try again.";
  if (code === "auth/network-request-failed") return "We could not reach the server. Check your connection and try again.";
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    const userData = await readUserProfile(user.uid);
    const userTier = userData.tier || "explorer";

    if (nameEl) nameEl.textContent = userData.displayName || user.displayName || user.email;
...
document.getElementById("nav-join")?.addEventListener("click", (event) => {
  event.preventDefault();
  sessionStorage.setItem("mimSelectedPlan", "explorer");
...
document.querySelectorAll(".auth-tab, .auth-link").forEach((element) => {
  element.addEventListener("click", () => setAuthTab(element.dataset.tab));
});
...
async function handleLogin() {
  ...
  if (!email || !password) {
    setError("login-error", "Enter both your email and password.");
    return;
  }

  authActionInProgress = true;
  setButtonBusy(button, true, "Logging In…", "Log In");

  try {
    const credential = await signInWithEmailAndPassword(auth, email, password);
    const userData = await readUserProfile(credential.user.uid);
    const displayName = userData.displayName || credential.user.displayName || "";

    setStatus("login-status", "Login successful. Loading your dashboard…");
    ...
    setError("login-error", friendlyAuthError(error, "login"));
  } finally {
    authActionInProgress = false;
    setButtonBusy(button, false, "Logging In…", "Log In");
  }
}
...
async function handleSignup() {
  ...
  const selectedPlan = sessionStorage.getItem("mimSelectedPlan") || "explorer";

  setError("signup-error");
  setStatus("signup-status");

  if (!name) {
    setError("signup-error", "Enter the name you want the crew to call you.");
    return;
  }
  if (!email) {
    setError("signup-error", "Enter your email address.");
    return;
  }
  if (password.length < 6) {
    setError("signup-error", "Your password must contain at least six characters.");
    return;
  }

  authActionInProgress = true;
  setButtonBusy(button, true, "Creating Account…", "Create Your Account");
  ...
  try {
    ...
    try {
      ...
      console.error("Account created, but Firestore profile save failed:", profileError);
      setStatus(
        "signup-status",
    ...
    sessionStorage.setItem("mimPostSignupPlan", selectedPlan);
    authModal?.classList.remove("active");
    if (authModal) authModal.style.display = "none";
    redirectToDashboard();
  } catch (error) {
    console.error("Signup failed:", error);
    setError("signup-error", friendlyAuthError(error, "signup"));
  } finally {
    authActionInProgress = false;
    setButtonBusy(button, false, "Creating Account…", "Create Your Account");
  }
}
